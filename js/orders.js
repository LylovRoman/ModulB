...
fetch('http://yarko.ct25692.tw1.ru/api/me', {
    ...
    .then((data) => {
        if (data.data.role == 'Мастер') {
            isMaster = true;
            name = data.data.name;
        }
        if (data.data.role == 'Клиент') {
            isClient = true;
            name = data.data.name;
            if (window.location.search.replace( '?myOrders=', '') != 'true'){
                window.location.href = window.location.search + '?myOrders=true';
            }
            ...
            fetch('http://yarko.ct25692.tw1.ru/api/order', {
                ...
                .then((data) => {
                    ...
                    data.forEach(el => {
                        if ((window.location.search.replace( '?myOrders=', '') != 'true') || (isMaster && (name == el.master)) || (isClient && (name == el.client.name))){
                            ...
                            row.appendChild(column);
                            column = document.createElement('td');
                            column.innerHTML = el.master ? el.master : "не назначен";
                            column.classList.add('border-slate-300');
                            column.classList.add('border');
                            column.classList.add('border-dark');
                            column.classList.add('text-center');
